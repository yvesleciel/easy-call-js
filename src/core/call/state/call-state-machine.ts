...
/**
 * Discrete states the call lifecycle can occupy. State transitions are
 * validated by {@link CallStateMachine}; illegal transitions raise a
 * {@link CallStateError}.
 */
...
/**
 * Data attached to the current call state — surfaced on every
 * {@code StateChanged} {@link CallEvent}.
 */
...
/**
 * Finite-state machine tracking the call lifecycle. Exposes the current
 * {@link CallState} and a stream of transitions consumed by
 * {@link CallProcessService} to publish {@code StateChanged} events.
 */
